...
if __name__ == '__main__':
    ...
    for t in tqdm(range(n_Tsamples)):
        ...
        with torch.no_grad():
            ...
            for x, _ in tqdm(test_dataloader):
                ...
                if VISUALIZE:
                    attention_ori = y['attention']
                    # print(x['sentence_ids'][0])
                    # tokens = wid2str[x['sentence_ids'][0]]
                    tokenizer = AutoTokenizer.from_pretrained(processor.language_model)
                    tokens = tokenizer.convert_ids_to_tokens(x['word_ids'][0])

                    # From subtokens to tokens
                    sub_ix =  [ [ix_j for ix_j, j in enumerate(x['subword_indices'][0].tolist()) if j==i ] for i in range(1, x['subword_indices'].max().item()+1) ] + [[ix_j for ix_j, j in enumerate(x['subword_indices'][0].tolist()) if j==0 ] ]
                    tokens = ["".join([tokens[j] for j in i]).replace("#", "") for i in sub_ix]
                    all_fix = torch.LongTensor([ix for ix, i in enumerate(sub_ix) for _ in i]).cuda()
                    attention = [scatter_mean(scatter_mean(i, all_fix, dim=-1), all_fix, dim=-2) for i in attention_ori]
                    # AVERAGED
                    # attention = [attention[i].mean(dim=1) for i in [0, 6, 12, 18]]

                    # model_view(attention, tokens)
                    # sub_id
                    # sel_ids = [(x['subword_indices'][0] == i+2).nonzero(as_tuple=True)[0] for i in x['instance_ids'][0].keys()] # batch size == 9
                    # sel_ids = [j.item() for i in sel_ids for j in i]

                    sel_ids = [i+1 for i in x['instance_ids'][0].keys()]
                    
                    att_ids = attention[-1][0,10][sel_ids, :].topk(k=5, dim=-1)[1]
                    att_tokens = [[tokens[att_ids[i,j]] for j in range(att_ids.shape[1])] for i in range(att_ids.shape[0]) ]
                    sel_tokens = [x['sentence_ids'][0]+'_'+tokens[i] for i in sel_ids]
                    for sel, att in zip(sel_tokens, att_tokens):
                        weighted_topK[sel] = att
        ...
        att_df = pd.DataFrame()
        att_df['instance'] = weighted_topK.keys()
        for i in range(5):
            att_df['top_attended_%d'%i] = [weighted_topK[j][i] for j in att_df['instance']]
        att_df.to_csv("results3/07_top_attended_instance_head10.csv", index=False)
        sys.exit(0)
    ...
    if n_Tsamples != 1:
        ...
        df['sense_0'] = [candidate_synsets[k[0]][best_inx[i]] for i,k in enumerate(predictions_scores_mean)]
        ...
        print(">>> Save to: " + "results2/MU/%s_UE_prediction_MC_T%d_seed%d.csv"%(args.mark, n_Tsamples,args.rand_seed))
        df.to_csv("results2/MU/%s_UE_prediction_MC_T%d_seed%d.csv"%(args.mark, n_Tsamples,args.rand_seed), index=False)
    ...
    # to save top 5 proper senses according to SR.
        ...
        print("Save to: "+ "results2/MU/%s_UE_prediction_SR_seed%d.csv"%(args.mark, args.rand_seed))
        df_SR.to_csv("results2/MU/%s_UE_prediction_SR_seed%d.csv"%(args.mark, args.rand_seed), index=False)
